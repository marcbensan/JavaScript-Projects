...
function toCamelCase(name, uppercase) {
  ...
  for (i; i < splitName.length; i++) {
    ...
    if (word !== '') {
      splitName[i] = word[0].toUpperCase() + word.slice(1).toLowerCase();
    }
  }
...
function createLinkTag(rel, href) {
  ...
  const link = `<link rel=${newRel} href=${newHref}>`;
  return link;
}
 ...
 *
 *
 * @param {string} dateArr - a date string
 * @returns {Date}
 ******************************************************************************/
...
function parseDateString(value) {
  try {
    ...
    if (dateArr[1].length == 2) {
      ...
      if (dateArr[0].length == 4 && dateArr[2].length == 2) {
        date.setFullYear(dateArr[0]);
        date.setDate(dateArr[2]);
        return date;
      } else if (dateArr[2].length == 4 && dateArr[0].length == 2) {
...
function toDateString(value, format) {
  ...
  const newYear = year.toLocaleString('en-US', { minimumIntegerDigits: 4, useGrouping: false });
  const newMonth = month.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false });
  const newDay = day.toLocaleString('en-US', { minimumIntegerDigits: 2, useGrouping: false });

  if (format === 'YYYY/MM/DD') {
    const dateArr = [newYear, newMonth, newDay].join('/');
    return dateArr;
  } else if (format === 'DD/MM/YYYY') {
    const dateArr = [newDay, newMonth, newYear].join('/');
    return dateArr;
  } else if (format === 'MM/DD/YYYY') {
    const dateArr = [newMonth, newDay, newYear].join('/');
    return dateArr;
  } else {
...
function normalizeTime(value) {
  const timeModifier = value.split(' ');
  const time = value.split(':');
  const deleteModifier = time[2].split(' ').slice(0, -1);
  const seconds = parseInt(time[2]);

  if (time[0] > 23) {
    return null;
  } else if (timeModifier[1] && time[0] > 12) {
    return null;
  } else if (time[1] > 59 || seconds > 59) {
    return null;
  }
  ...
  if (timeModifier[1] === 'PM') {
    ...
    const finalTime = [
      time[0],
      time[1],
      deleteModifier.map((item) => item.replace(/^0+/, '0'))
    ].join(', ');
    return `(${finalTime})`;
  } else if (timeModifier[1] === 'AM') {
    const finalTime = [
      time[0],
      time[1],
      deleteModifier.map((item) => item.replace(/^0+/, '0'))
    ...
    const finalTime = [time[0], time[1], time[2]].join(', ');
...
function formatTimes(...values) {
  const formattedValues = values.map(normalizeTime).filter((time) => time !== null);
...
function mimeTypeFromFilename(filename) {
  ...
  const mimeTypes = {
    ...
    xml: 'application/xml',
    csv: 'text/csv'
  }; // i created a library that has a key:value pairs
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  if (resourceType.length === 0 || resourceId.length === 0) {
    throw new Error("both arguments can't be empty");
  }

  if (extraData === null || extraData === undefined) {
    return `/${resourceType}/${resourceId}`;
  } else if (extraData.length === 0) {
    throw new Error('extraData must not be empty.');
  } else {
